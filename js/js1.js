...
let arrayOfRandomNumber = getArrayOfRandomNumber(getChecked());
...
for(let i = 0; i < radios.length; i++) {
    radios[i].addEventListener('change', go)
}

function go(event) {
    let next = confirm('Прогресс будет потерян. Начать новую игру?');
    if (next) {
    createElement();

    arrayOfRandomNumber = getArrayOfRandomNumber(getChecked());
    getQuestionElem();
    answerElems[0].focus();
    }
}
...
function createElement() {
    let resultsElems = document.querySelectorAll('p');
...
function getQuestionElem(){
    for (let i = 0; i < questionElems.length; i++) {
        questionElems[i].value = arrayOfRandomNumber[i];
    }
}
...
function runCheck () {
    if(inputValidation(answerElems)) {
        ...
        p.textContent = countStep + '. ' + answer.join(' ') + '. Коров: ' + cows + ', быков: ' + bulls;
